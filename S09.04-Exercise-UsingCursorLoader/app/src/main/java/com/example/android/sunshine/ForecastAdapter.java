...
//      Done (7) Generate a weather summary with the date, description, high and low
        ...
        long dateIndex = mCursor.getLong(MainActivity.INDEX_WEATHER_DATE);
        String dateString = SunshineDateUtils.getFriendlyDateString(mContext, dateIndex, false);
        double MinTempC = mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP);
        double MaxTempC = mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);
        String MaxAndMinTemp = SunshineWeatherUtils.formatHighLows(mContext, MaxTempC, MinTempC);

        String weatherSummary = dateString + " - " + description + " - " + MaxAndMinTemp;
...
//      Done (10) If mCursor is null, return 0. Otherwise, return the count of mCursor
     ...
     *
     * @param cursor The new weather data to be displayed.
     */
